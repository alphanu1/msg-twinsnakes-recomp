...
| `tools/patches/DolRecomp-native-services.patch` | `github.com/ExpansionPak/DolRecomp` @ `71ce7f97419b1bb1ba9a9596c41507f6629e0fb0` | GPL-3.0 | `src/app/pipeline.c`: with `DOLRECOMP_NATIVE_SERVICES=1`, the exception and memory-service blockers on the native call ABI are lifted for the default runtime as upstream lifts them for ModernGekko's. Without it, every function with a generic memory access was not natively callable, so almost every call between guest functions left native code for the run loop and came back. Valid because this host's `external_read`/`external_write` never raise a guest exception and never read guest registers (HANDOFF F366). |
