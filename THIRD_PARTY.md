...
| `imgui` | **MIT** | linked | Dear ImGui, `ocornut/imgui` @ `f1cc2ae15e53a861a874c3034aae6798fde194ab` (v1.92.9b). The launcher's screens (design document; Ben, 2026-09-25). Unmodified; compiled from `extern/imgui` with its SDL3 and SDL_Renderer3 backends. |
| SDL3 (binary release) | **zlib** | shipped | `libsdl-org/SDL` release 3.4.14: built from source in the Steam Runtime image for the Linux release, and the official `SDL3-devel-3.4.14-mingw.tar.gz` for the Windows release, whose `SDL3.dll` ships beside the game (zlib licence: redistribution allowed, notice kept). Unmodified. |
...
| `tools/patches/DolRecomp-pipeline-switches.patch` | `github.com/ExpansionPak/DolRecomp` @ `71ce7f97419b1bb1ba9a9596c41507f6629e0fb0` | GPL-3.0 | `src/app/pipeline.c`, four switches: `DOLRECOMP_NATIVE_SERVICES=1` lifts the native-call ABI's exception and memory-service blockers for the default runtime (tried and withdrawn, F366 - kept, off); `DOLRECOMP_LLVM_OPT_LEVEL` (was hard-wired to 3; this project uses 2); `DOLRECOMP_NO_THINLTO=1` skips the bitcode summaries written beside every object, which nothing here reads; every environment switch any local patch reads is made part of the object-cache key, which before this quietly reused objects built with different switches (F369); `DOLRECOMP_ENTER_DATA_REFS=1` makes every aligned data word holding a code address an entry point (jump tables, function pointers), `DOLRECOMP_EXTRA_ENTRIES=<file>` adds entries listed by the caller (F370); `DOLRECOMP_EXTERNAL_RANGES=<file>` gives the emitter another binary's function ranges so calls into it are direct (F386). |
| `tools/patches/DolRecomp-entry-guards.patch` | `github.com/ExpansionPak/DolRecomp` @ `71ce7f97419b1bb1ba9a9596c41507f6629e0fb0` | GPL-3.0 | New `src/backend/llvm/entry_guard.{h,cpp}`, one line in `exits.cpp`, one in `CMakeLists.txt`: with `DOLRECOMP_GUARDED_ENTRIES=<file>` only the functions listed get the entry-time interception query (`ppc_native_region_available`); every other native function entry goes straight in. The list is this project's patch table (F384). The pipeline-switches patch adds the variable to the object-cache key. |
...
| AX's compressor: over threshold in a 5 ms frame, one attack ramp chosen by the release position, then the release ramps; output clamped to +-32767 after it | `host/ax_dsp.c`, `ax_compress` (parameters from the AX library's command, community decomp `dolsdk2004` `AXCL.c`; the ramp table read from guest memory) | `Source/Core/Core/HW/DSPHLE/UCodes/AX.cpp`: `AXUCode::RunCompressor`, `OutputSamples` |
| Which outputs a voice feeds: newer-microcode `mixerCtrl` bits (main left 0x0001, right 0x0002) | `host/ax_dsp.c`, the `mixerCtrl` block | `Source/Core/Core/HW/DSPHLE/UCodes/AX.cpp`: `AXUCode::ConvertMixerControl` |
