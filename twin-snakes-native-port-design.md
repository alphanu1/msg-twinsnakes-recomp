...
**The graphics processor gets its own host thread (2026-09-25).** *Believed until now:* no host thread touches guest memory except through queued events, so the whole GX pipeline - FIFO parse, vertex decode, transform, GPU submission, EFB copies - ran on the game's one thread, inline with every gather-pipe write. *Now known:* in gameplay that pipeline is about a quarter of the thread the whole game runs on (HANDOFF F359, F360), and the console never did that work on the CPU at all. The Flipper consumes its FIFO concurrently and reads display lists, vertex arrays and textures from main memory by DMA while the CPU carries on; the game keeps that memory stable until the draw-done token because it must on hardware too. *So:* the gather pipe feeds a ring, and a render thread parses it (`host/display.c`) - the one host thread besides DVD completion that reads and writes guest memory directly, and it does so where and when the graphics processor does. What crosses back to the game is what crosses back on the console: the draw-done token, counted by the parser and delivered as an interrupt on the game's own thread, and copy results in main memory, which the game reads after that token. Guest *threads* still run one at a time; this changes nothing about the game's atomicity. Hardware behaviour is the authority here (rule 12, order 1). `MGS_GX_SYNC=1` restores inline parsing for comparison.
...
| DSP init, EXI, SI | Low-level bus setup | **Not stubs — modelled registers.** Believed to be "stubs that return success, Low" until phase 0 ran it. Three of these turned out load-bearing and each was found by a boot that stopped: EXI because the memory card cannot be stubbed at the SDK's API (F166–F170) and has to answer as a device down to its SRAM checksum; SI because a controller that never finishes a transfer never appears (F153); and DSP because its interrupts pace the machine. The audio DMA's completion is what asks the game for the next buffer of sound, so with it unmodelled a streaming game does not run silently, it **stops** — and stops reading the disc, having nothing to read for (F187). A success stub cannot reproduce an interrupt, and F184 showed the stubs cannot simply be withdrawn either. **F236 extends this one level deeper, and it is the same finding, not a new one:** it is not only the DSP's *interrupts* that pace the machine but the AX voice's *playback position*. `sd_stream_pump` reads `pb.addr.currentAddress` out of each voice and asks the engine for more audio only as that field moves; the DSP owns it, so with no DSP it never moves and a streamed movie freezes with its video records decoded and unread. `host/ax_dsp.c` advances that one field per AX frame and **mixes nothing** — which unfroze the movie. The phase-4 item below is still the real voice mixer; this is pacing, and it lands here for the same reason the interrupts do. | **Medium**, and the ordering matters: these block boot, so they land in phase 0–1, not with audio in phase 4 |
...
| Threading bugs from cooperative-to-host mismatch | Medium | Single guest thread at a time, always; no host thread touches guest memory except through queued events - **with one exception, the render thread, which reads and writes it as the graphics processor does and synchronises with the game through the draw-done token (see Threads)** |
